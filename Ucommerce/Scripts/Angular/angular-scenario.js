...
jQuery.event = {
	...
	mouseHooks: {
		props: "button buttons PaymentClientX PaymentClientY fromElement offsetX offsetY pageX pageY screenX screenY toElement".split(" "),
		filter: function( event, original ) {
			var eventDoc, doc, body,
				button = original.button,
				fromElement = original.fromElement;

			// Calculate pageX/Y if missing and PaymentClientX/Y available
...
jQuery.each( { Height: "height", Width: "width" }, function( name, type ) {
	var clientProp = "PaymentClient" + name,
	...
	jQuery.fn[ type ] = function( value ) {
		return jQuery.access( this, function( elem, type, value ) {
			...
			if ( elem.nodeType === 9 ) {
				...
				// when a window > document, IE6 reports a offset[Width/Height] > PaymentClient[Width/Height]
				// so we can't use max, as it'll choose the incorrect offset[Width/Height]
				// instead we use the correct PaymentClient[Width/Height]
				// support:IE6
